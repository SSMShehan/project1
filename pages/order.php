...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['create'])) {
        ...
        if (!validateCustomerExists($conn, $customerID)) {
            ...
            $sql = "INSERT INTO order_details (Customer_ID, Item_ID, Price, Quantity, Date_created, Date_updated) 
                    VALUES (?, ?, ?, ?, NOW(), NOW())";
            
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("iidi", $customerID, $itemID, $price, $quantity);
                if ($stmt->execute()) {
        ...
        if (!validateCustomerExists($conn, $customerID)) {
            ...
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("iidii", $customerID, $itemID, $price, $quantity, $orderID);
                if ($stmt->execute()) {
...
$count_result = $conn->query("SELECT COUNT(*) AS total FROM order_details");
$total_records = $count_result->fetch_assoc()['total'];
$total_pages = ceil($total_records / $records_per_page);

// Fetch paginated order details
$sql_order_details = "SELECT * FROM order_details LIMIT $offset, $records_per_page";
...
function openEditModal(orderId, customerId, itemId, price, quantity) {
