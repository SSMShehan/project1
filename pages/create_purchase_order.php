...
$products = [];
$prod_result = $conn->query("SELECT p.product_id, p.sku, p.name, p.unit_price, p.cost_price, c.name as category_name 
                           FROM products p 
...
if ($is_edit) {
    ...
    if (!$po_data) {
        ...
        // Load PO items
        $items_stmt = $conn->prepare("SELECT poi.*, p.sku, p.name 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $items = [];
    $item_count = isset($_POST['product_id']) ? count($_POST['product_id']) : 0;
    ...
    if (empty($supplier_id)) {
        ...
        try {
            ...
            if ($is_edit) {
                ...
                $stmt->bind_param("issssi", $supplier_id, $order_date, $expected_delivery_date, $status, $notes, $po_id);
                $stmt->execute();
                $stmt->close();
                
                // Delete existing items
                $del_stmt = $conn->prepare("DELETE FROM purchase_order_items WHERE po_id = ?");
                ...
                $stmt = $conn->prepare("INSERT INTO purchase_orders 
                                      (po_number, supplier_id, order_date, expected_delivery_date, status, notes, created_by) 
                                      VALUES (?, ?, ?, ?, ?, ?, ?)");
                $user_id = $_SESSION['user_id']; // Make sure this is set
                error_log("User ID: " . $user_id); // Debugging
                
                $stmt->bind_param("sissssi", $po_number, $supplier_id, $order_date, $expected_delivery_date, $status, $notes, $user_id);
                $stmt->execute();
            ...
            // Calculate tax (10%) - this could be configurable
            $tax_rate = 0.10;
            $tax_amount = $subtotal * $tax_rate;
            $total_amount = $subtotal + $tax_amount;
            
            // Update PO with calculated amounts
            $update_stmt = $conn->prepare("UPDATE purchase_orders SET 
                                         subtotal = ?, 
                                         tax_amount = ?, 
                                         total_amount = ? 
                                         WHERE po_id = ?");
            $update_stmt->bind_param("dddi", $subtotal, $tax_amount, $total_amount, $po_id);
            $update_stmt->execute();
            $update_stmt->close();
            
            $conn->commit();
            
            $message = $is_edit ? 'Purchase order updated successfully!' : 'Purchase order created successfully!';
            $messageType = 'success';
            
            // Redirect to view page after creation
            if (!$is_edit) {
                error_log("Redirecting to purchase_order_details.php?id=$po_id");
            ...
            $message = 'Error saving purchase order. Please try again.';
            $messageType = 'error';
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title><?php echo $is_edit ? 'Edit' : 'Create'; ?> Purchase Order</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* Same styles as before, plus additional styles for item table */
        .form-container {
...
<div class="form-container">
    <h2><?php echo $is_edit ? 'Edit' : 'Create'; ?> Purchase Order</h2>
    ...
    <form method="POST" action="">
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="supplier_id" name="supplier_id" required>
                    ...
                    <?php foreach ($suppliers as $id => $name): ?>
                        <option value="<?php echo $id; ?>" <?php echo ($is_edit && $po_data['supplier_id'] == $id) ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <input type="date" id="order_date" name="order_date" required 
                       value="<?php echo $is_edit ? htmlspecialchars($po_data['order_date']) : date('Y-m-d'); ?>">
            </div>
            
            <div class="form-group">
                <label for="expected_delivery_date">Expected Delivery Date</label>
                <input type="date" id="expected_delivery_date" name="expected_delivery_date"
                       value="<?php echo $is_edit && $po_data['expected_delivery_date'] ? htmlspecialchars($po_data['expected_delivery_date']) : ''; ?>">
            </div>
            
            <div class="form-group">
                <label for="status">Status *</label>
                <select id="status" name="status" required>
                    <option value="draft" <?php echo ($is_edit && $po_data['status'] == 'draft') ? 'selected' : ''; ?>>Draft</option>
                    <option value="ordered" <?php echo ($is_edit && $po_data['status'] == 'ordered') ? 'selected' : ''; ?>>Ordered</option>
                    <?php if ($is_edit && $po_data['status'] == 'received'): ?>
                        <option value="received" selected>Received</option>
                    <?php endif; ?>
                    <?php if ($is_edit && $po_data['status'] == 'cancelled'): ?>
        ...
        <div class="form-group">
            ...
            <textarea id="notes" name="notes"><?php echo $is_edit ? htmlspecialchars($po_data['notes']) : ''; ?></textarea>
        </div>
                                ...
                                <select name="product_id[]" class="product-select" required>
                                    ...
                                    <?php foreach ($products as $id => $product): ?>
                                        <option value="<?php echo $id; ?>" 
                                            ...
                                            <?php echo ($item['product_id'] == $id) ? 'selected' : ''; ?>>
...
let itemCounter = <?php echo $is_edit ? count($po_items) : 0; ?>;
    ...
    tr.innerHTML = `
        <td>
            <select name="product_id[]" class="product-select" required>
                <option value="">-- Select Product --</option>
                <?php foreach ($products as $id => $product): ?>
                    <option value="<?php echo $id; ?>" data-price="<?php echo $product['cost_price']; ?>">
                        <?php echo htmlspecialchars($product['name'] . ' (' . $product['sku'] . ')'); ?>
                    </option>
                <?php endforeach; ?>
            </select>
...
// Update row total when product, quantity or price changes
function updateRow() {
...
document.addEventListener('change', function(e) {
    if (e.target.classList.contains('product-select') && e.target.value) {
        ...
        priceInput.value = product.cost_price.toFixed(2);
